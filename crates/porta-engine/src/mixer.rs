...
//! master. Parameter changes ramp linearly across the next block so jumps
//! never click (REQ-602). Playback-side only; nothing here touches tape.
...
pub struct Mixer {
    ...
    pan: [f32; NUM_TRACKS],
    master_db: f32,
    // Smoothed state: the gains the previous block ended on.
    current: [TrackGain; NUM_TRACKS],
}
...
impl Mixer {
    pub fn new() -> Self {
        ...
        // Start settled on the default targets so the first block does not
        // fade in from silence.
        for t in 0..NUM_TRACKS {
            m.current[t] = m.target(t);
        }
        m
    }
    ...
    fn target(&self, track: usize) -> TrackGain {
    ...
    /// Mix one block of the four track signals into stereo out. All slices
    /// must share the same length. Gains ramp linearly across the block
    /// from the previous block's end state to the current targets.
    pub fn mix_block(
        &mut self,
        ...
        for (t, input) in inputs.iter().enumerate() {
            ...
            let to = self.target(t);
            let step_l = (to.left - from.left) * inv;
            let step_r = (to.right - from.right) * inv;
            let mut gl = from.left;
            let mut gr = from.right;
...
mod tests {
    ...
    fn hard_pan_silences_the_other_side() {
        ...
        // Settle the ramp, then measure a second block.
        mix_once(&mut m, 2, &s);
